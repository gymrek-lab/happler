...
# arg7: 0 or 1 indicating whether to include the parent and child SNPs in the regression as covariates. Requires that the child SNP be the second node from the root of the tree. (optional - defaults to 0)
# ex: workflow/scripts/midway_manhattan.bash out/19_55363180-55833573/genotypes/snps.pgen 19_55363180-55833573.indep.pheno 19_55363180-55833573.indep.hap 19_55363180-55833573.indep H0 rs61734259 1
...
# now, retrieve the contents of the hap file that denote the haplotype up until the child SNP
...
if [ "$condition" -eq 1 ]; then
    ...
    last_arg="-a $(grep -P '^H\t.*\t'"$hap_id"'\t' "$hap_file" | cut -f7)"
fi

# step 3: use manhattan.py to actually create the manhattan plot
workflow/scripts/manhattan.py \
-i "$snp_id" \
-o "$out_prefix".png \
-l "$out_prefix".*.glm.linear \
$last_arg
